...
public class MiProveedorContenido
        ...
        switch (sURIMatcher.match(uri)){
            case 2:
                ...
                AppDataBase db =
                        AppDataBase.getDataBaseInstance(getContext());
                UserDao dao = db.getUserDao();
                List<User>  lstUserDel = dao.loadAllByIds(new int[]{idDelete});
                User userDel = lstUserDel.get(0);
                filasAfectadas = dao.delete(userDel);
                break;
        }
    ...
    public int update(@NonNull Uri uri,
                      @Nullable ContentValues contentValues, @Nullable String s, @Nullable String[] strings) {
